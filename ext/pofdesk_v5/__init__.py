...
#             fields.append(field)
        if self.path.startswith('/static/'):
            ...
            path=path_prase('template')
            render = template.render(path)
            s=render.protocol(self.Operation_argument,protocols,protocol,protocol_name,Switch_list,Table_entry,Metadata,Flow_entry_list,ports,Tables,Table,fields)           
            s=str(s)
            s=s.replace('&quot;', '"')#translate &quot into "
            self.send_response(200)
            self.send_header('Content-type','text/html')
...
###################to send message to HTML#################################
        path=path_prase('template')
        render = template.render(path)
        f=render.protocol(self.Operation_argument,protocols,protocol,protocol_name,Switch_list,Table_entry,Metadata,Flow_entry_list,ports,Tables,Table,fields)
        self.send_response(200)
        self.end_headers()
        self.wfile.write(f)
class tablehandler(StaticContentHandler):
    ...
    def do_GET(self):
        ...
        if self.path.startswith('/static/'):
            ...
            path=path_prase('template')
            render = template.render(path)
            s=render.table(showflowentrys,Switch_list,protocols,showtables)           
            s=str(s)
            s=s.replace('&quot;', '"')#translate &quot into "
            self.send_response(200)
            self.send_header('Content-type','text/html')
    ...
    def do_POST (self):
        ...
        path=path_prase('template')
        render = template.render(path)
        f=render.table(showflowentrys,Switch_list,protocols,showtables)
        self.send_response(200)
        self.end_headers()
        self.wfile.write(f)
class porthandler(StaticContentHandler):
    def do_GET(self):
        ...
        render = template.render(path)
        try:
            s=render.port(jsonStr,Switch_list)
            s=str(s)
            s=s.replace("&#39;","'")
            s=s.replace('&quot;','"')
            self.send_response(200)
    ...
    for instruction in instructions:
        ...
        if instruction['action']=="apply_action":
            ...
            if instruction['action_type']=="set_field":
                ...
                field_id=int(string.split(instruction["name"],':')[0])
            ...
            if instruction['action_type']=="add_field":
                ...
                print instruction
                if instruction["Existed_Field"]:
                    field_id=int(string.split(instruction["Existed_Field"],':')[0])
                    field=core.PofManager.get_field(field_id)
                    field_position=field.offset
                    field_length=field.length
                else:
                    field_name=instruction["New_Field_Name"]
                    if instruction["field_offset"]:
                        field_position=int(instruction["field_offset"])
                    else:
                        field_position=0
                    if instruction["field_length"]:
                        field_length=int(instruction["field_length"])
                    else:
                        field_length=0
                    field_id = core.PofManager.new_field(field_name, field_position, field_length)
            ...
            if instruction['action']=="goto_table":
                ...
                print Table_entry
                switch_id=OnlyStr(Table_entry["switch_id"])
