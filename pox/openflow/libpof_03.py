...
ofp_table_mod_cmd_rev_map = {  # edit by CC
    'OFPTC_ADD'    : 1,
    'OFPTC_MODIFY' : 2,
    'OFPTC_DELETE' : 3,
}
...
class ofp_header (ofp_base):
    ...
    def show (self, prefix=''):
        ...
        outstr += prefix + 'Ver=' + str(self.version)
    ...
    def __str__ (self):
        return "[" + self.__class__.__name__ + "]" + self.show(' ').strip()
...
class ofp_instruction_goto_table(ofp_instruction_base):
    """
    edit by CC
    according to org.openflow.protocol.instruction.OFInstructionGotoTable
    """
...
class ofp_packet_in (ofp_header):
    ...
    def pack (self):
        ...
        packed = b""
        packed += ofp_header.pack(self)
        packed += struct.pack("!LHBBQLHH", self._buffer_id, self.total_len, self.in_port,
    ...
    def unpack (self, raw, offset=0):
        ...
        offset, (self._buffer_id, self._total_len, self.reason, self.table_id, self.cookie,
            self.device_id, self.slot_id, self.port_id) = _unpack("!LHBBQLBB", raw, offset)
